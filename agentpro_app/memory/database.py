...
CREATE TABLE IF NOT EXISTS users (
    ...
    preferences TEXT  -- JSON blob for flexibility
);
...
CREATE TABLE IF NOT EXISTS quiz_attempts (
    ...
    timestamp TEXT NOT NULL,
    answers TEXT,  -- JSON array of detailed answers
    FOREIGN KEY (user_id) REFERENCES users(user_id),
    FOREIGN KEY (course_id) REFERENCES courses(course_id)
);
...
CREATE TABLE IF NOT EXISTS mastery_scores (
    ...
    recent_scores TEXT,  -- JSON array of last 10 scores
...
def log_quiz_attempt(
    ...
    with get_db() as conn:
        ...
        # Update topic status (weak/moderate/strong)
        cursor = conn.execute("""
...
def get_stats(user_id: str, course_id: str) -> Dict:
    ...
    with get_db() as conn:
        ...
        """, (user_id, course_id)).fetchone()['cnt']
        ...
        if len(recent_5) >= 3:
            ...
            older_avg = sum(r['score'] for r in older_5) / len(older_5) if older_5 else recent_avg
    ...
    with get_db() as conn:
        if topic:
            rows = conn.execute("""
                ...
                WHERE user_id = ? AND course_id = ? AND topic = ?
                ORDER BY timestamp DESC LIMIT ?
            """, (user_id, course_id, topic, limit)).fetchall()
        else:
            rows = conn.execute("""
                SELECT * FROM quiz_attempts
                WHERE user_id = ? AND course_id = ?
